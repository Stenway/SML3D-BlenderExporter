...
class Sml3DFileWriter:
    ...
    def write_root(self):
        self.writer.begin_element('SML3D')
        
        self.writer.string_attribute('Version', '0.1.0')
    ...
    def write_meta(self):
        ...
        self.writer.string_attribute('FilePath', bpy.data.filepath)
        now = datetime.now()
    ...
    def write_material(self, material):
        self.writer.begin_element('Material')
        ...
        if texture:
            self.writer.string_attribute('DiffuseMap', texture)
    ...
    def write_node(self, obj):
        ...
        self.writer.string_attribute('Name', obj.name)
...
class Sml3DExporter(Operator, ExportHelper):
    """SML3D Exporter"""
    bl_idname = 'stenway.sml3d_export'
    bl_label = 'Export SML3D'
...
#    )
    ...
    def execute(self, context):
        writer = Sml3DFileWriter(self.filepath, ReliableTxtEncoding("utf-8"), self.triangulate)
        return {'FINISHED'}


def menu_func_export(self, context):
    self.layout.operator(Sml3DExporter.bl_idname, text='SML3D (.sml3d)')


def register():
    bpy.utils.register_class(Sml3DExporter)
    bpy.types.TOPBAR_MT_file_export.append(menu_func_export)


def unregister():
    bpy.utils.unregister_class(Sml3DExporter)
...
if __name__ == '__main__':
    ...
    bpy.ops.stenway.sml3d_export('INVOKE_DEFAULT')
